...
interface FooterProps {
  settings?: {
    logoWhite?: { asset?: { url?: string } };
    nacascoloLogo?: { asset?: { url?: string } };
    socialLinks?: Array<{ _key: string; platform: string; url: string }>;
  } | null;
}

export default function Footer({ settings }: FooterProps = {}) {
  const logoWhiteSrc = settings?.logoWhite?.asset
    ? urlFor(settings.logoWhite).width(200).height(120).url()
    : '/images/nb logo white lowdef-640w.png';
  const nacascoloSrc = settings?.nacascoloLogo?.asset
    ? urlFor(settings.nacascoloLogo).width(260).height(90).url()
    : '/images/nacas logo-640w.png';
  const socialLinks = settings?.socialLinks?.length
    ? settings.socialLinks.map((l) => ({ platform: l.platform, url: l.url }))
    : defaultSocialLinks;
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex-shrink-0">
            <Image
              ...
              unoptimized={!!settings?.logoWhite?.asset}
            />
          </div>
          ...
          <div className="flex-shrink-0">
            <Image
              ...
              unoptimized={!!settings?.nacascoloLogo?.asset}
            />
          </div>
            ...
            {socialLinks.map((link) => (
              ...
              >
                {socialIcons[link.platform] || (
                  <span className="w-5 h-5 inline-block text-xs">{link.platform}</span>
                )}
              </a>
            ))}
          </div>
