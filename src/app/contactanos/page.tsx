...
import { SITE_SETTINGS_QUERY, CONTACTANOS_PAGE_QUERY } from '@/sanity/lib/queries';
...
const defaultContactItems = [
  { type: 'address', label: 'Dirección', value: 'San José, Costa Rica' },
  { type: 'email', label: 'Email', value: 'info@natural-bites.com' },
  { type: 'phone', label: 'Teléfono', value: '+506 0000-0000' },
];
...
export default async function Contactanos() {
  const [settings, pageData] = await Promise.all([
    client.fetch(SITE_SETTINGS_QUERY),
    client.fetch(CONTACTANOS_PAGE_QUERY),
  ]);

  const contactItems = settings?.contactItems && settings.contactItems.length > 0
      ...
      <section className="bg-[#f5c964] py-16 px-4">
        <div className="max-w-6xl mx-auto text-center">
          <h1 className="text-4xl md:text-5xl font-bold text-[#333] mb-4">
            {pageData?.heroHeading ?? 'CONTÁCTANOS'}
          </h1>
          <p className="text-lg text-[#333]">
            {pageData?.heroSubtitle ?? 'Estamos aquí para ayudarte'}
          </p>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-12">
            ...
            <div>
              <h2 className="text-2xl font-bold text-[#333] mb-6">
                {pageData?.formHeading ?? 'Envíanos un mensaje'}
              </h2>
              <form className="space-y-6">
                ...
                <button type="submit" className="btn-primary w-full">
                  {pageData?.formButtonText ?? 'Enviar mensaje'}
                </button>
              </form>
            </div>
            ...
            <div>
              <h2 className="text-2xl font-bold text-[#333] mb-6">
                {pageData?.contactInfoHeading ?? 'Información de contacto'}
              </h2>
              <div className="space-y-6">
                {contactItems.map((item: { _key?: string; type: string; label: string; value: string }, index: number) => (
                  <div key={item._key ?? index} className="flex items-start gap-4">
                    <div className="w-12 h-12 bg-[#f5c964] rounded-full flex items-center justify-center flex-shrink-0">
                      {contactIcons[item.type] ?? contactIcons.address}
                    </div>
                    <div>
                      <h3 className="font-semibold text-[#333]">{item.label}</h3>
                      <p className="text-[#666]">{item.value}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-8">
                <h3 className="font-semibold text-[#333] mb-4">
                  {pageData?.socialLinksHeading ?? 'Síguenos'}
                </h3>
                <div className="flex gap-4">
                  {socialLinks.map((link: { _key?: string; platform: string; url: string }, index: number) => (
                    <a
                      key={link._key ?? index}
                      href={link.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-10 h-10 bg-[#f5c964] rounded-full flex items-center justify-center hover:bg-[#e5b54e] transition-colors"
                    >
                      {socialIcons[link.platform] ?? socialIcons.facebook}
                    </a>
                  ))}
                </div>
              </div>
